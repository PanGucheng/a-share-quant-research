...
每次只运行当前阶段。交互式 PowerShell 的前一条命令 throw，不保证停止已经粘贴排队的下一条命令。新版入口对 BaoStock 10002007 和 HTTP 连接/超时错误自动重试：等待30、60、120秒，后续上限120秒；每次启动总计最多100次重试（不是每只股票100次；首次尝试之外最多重试100次）。失败连接关闭后重新登录，并复用完成块，为失败处新建 attempt。权限、TLS证书、数据/日期/hash错误立即停止；Ctrl+C 在等待期间也直接停止。达到上限后保留日志，不删除 receipt。可通过 -MaxRetries、-RetryDelaySeconds、-MaxDelaySeconds 调整等待预算，-MaxRetries 0 关闭自动重试。
