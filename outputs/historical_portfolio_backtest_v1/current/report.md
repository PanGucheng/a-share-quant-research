...
- `portfolio_holdout_supported_relative_advantage=false`；无论结果正负，参数均未改变。
- holdout gross return approximation 为 `9.37%`，基准收益为 `19.19%`；加回成本后仍低于基准：`true`。
- 实际持仓使用 stale valuation fallback 的日期数为 `0`。当前首要研究方向：模型与组合在不同市场状态下的稳定性和风格暴露；第二研究方向：降低换手与交易成本。
