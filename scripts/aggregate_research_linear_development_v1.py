...
from model_research.linear_aggregate import (  # noqa: E402
    aggregate_linear_development,
)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Aggregate the complete Ridge and Elastic Net development evidence."
    )
    parser.add_argument(
        "--config",
    ...
    parser.add_argument(
        "--output-dir",
        default="outputs/research_linear_models_v1/development",
    )
    args = parser.parse_args()
    result = aggregate_linear_development(
        load_linear_config(resolve(args.config)),
        output_dir=resolve(args.output_dir),
        command=" ".join(shlex.quote(value) for value in sys.argv),
    )
    print(
        "Linear development aggregate passed: "
