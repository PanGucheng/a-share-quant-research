...
DEFAULT_CONFIG = Path("configs/alpha101_factor_smoke_promotion_v1.yaml")
...
def promote_catalog(config_path: Path) -> Path:
    ...
    for factor in selected_factors:
        ...
        if passed and entry is not None:
            entry["stage"] = str(promotion.get("stage", "alpha101_adapter_v4_smoke_passed"))
            entry["enabled"] = bool(promotion.get("enabled", True))
            entry["runnable"] = bool(promotion.get("runnable", True))
            entry["notes"] = f"{entry.get('notes', '')} V4 smoke passed for selected Alpha101 adapter factor.".strip()
    ...
    output_payload = {
        ...
        "updated": "2026-06-26",
        "policy": {
            "purpose": "Promoted KunQuant Alpha101 smoke factors after V4 adapter evaluation.",
            "principle": [
                "This is a smoke-level runnable catalog, not the full Alpha101 source catalog.",
                "Formula definitions are sourced from KunQuant pandas reference code.",
    ...
    print(f"Alpha101 smoke promotion catalog written to {output_catalog_path}", flush=True)
    return output_catalog_path


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Promote selected Alpha101 smoke factors after V4 evaluation.")
