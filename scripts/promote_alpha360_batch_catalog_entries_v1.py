...
DEFAULT_CONFIG = Path("configs/alpha360_factor_batch_promotion_v1.yaml")
...
def update_entry(entry: dict, *, stage: str, enabled: bool, runnable: bool, suffix: str) -> dict:
    ...
    notes = str(result.get("notes", "")).replace("Pending Alpha360 batch V4 evaluation.", "").strip()
...
def run(config_path: Path) -> dict[str, Path]:
    ...
    adapter_holdout_source_path = resolve_path(config["adapter_holdout_catalog"])
    batch_root = resolve_path(config["batch_root"])
    batch_passed_path = resolve_path(config["batch_passed_catalog"])
    v4_holdout_path = resolve_path(config["v4_holdout_catalog"])
    all_holdout_path = resolve_path(config["all_holdout_catalog"])
    ...
    adapter_holdout_source = load_yaml(adapter_holdout_source_path)
    source_entries = {str(item["name"]): dict(item) for item in source.get("factors", [])}
    adapter_holdout_entries = [dict(item) for item in adapter_holdout_source.get("factors", [])]
    ...
    for factor, entry in source_entries.items():
        ...
        if decision == "promoted":
            promoted_entries.append(
                update_entry(
                    entry,
                    stage=str(promotion.get("passed_stage", "alpha360_adapter_v4_batch_passed")),
                    enabled=True,
                    runnable=True,
                    suffix="Alpha360 batch V4 passed.",
                )
            )
        else:
            v4_holdout_entries.append(
                update_entry(
                    entry,
                    stage=str(promotion.get("holdout_stage", "alpha360_adapter_v4_batch_holdout")),
                    enabled=False,
                    runnable=False,
                    suffix=f"Alpha360 batch V4 holdout: {reason}.",
                )
            )
        audit_rows.append(
            {
    ...
    common_policy = {
        ...
        "principle": [
            ...
            "Adapter constant identity factors remain holdouts and are not sent to V4 batch.",
        ],
    }
    write_yaml(
        {
            "version": 1,
            "updated": "2026-06-28",
            "policy": {**common_policy, "purpose": "Alpha360 batch-passed factors after candidate358 V4 evaluation."},
    ...
    write_yaml(
        {
            "version": 1,
            "updated": "2026-06-28",
            "policy": {**common_policy, "purpose": "Alpha360 V4 holdout factors after candidate358 batch evaluation."},
            "factors": v4_holdout_entries,
        },
        v4_holdout_path,
    )
    all_holdouts = sorted(adapter_holdout_entries + v4_holdout_entries, key=lambda item: str(item["name"]))
    write_yaml(
        {
            "version": 1,
            "updated": "2026-06-28",
            "policy": {**common_policy, "purpose": "Combined Alpha360 holdout catalog: adapter constants plus V4 holdouts."},
            "factors": all_holdouts,
        },
        all_holdout_path,
    )
    write_yaml(
        {
            "version": 1,
            "updated": "2026-06-28",
            "policy": {**common_policy, "purpose": "Combined Alpha360 promoted catalog after batch V4 evaluation."},
            "factors": sorted(promoted_entries, key=lambda item: str(item["name"])),
        },
        combined_path,
    )
    ...
    lines = [
        "# Alpha360 Batch Promotion V1",
        "",
        f"- Batch root: `{batch_root.as_posix()}`",
        f"- Source factors: `{len(source_entries)}`",
        f"- Batch promoted: `{len(promoted_entries)}`",
        f"- V4 batch holdout: `{len(v4_holdout_entries)}`",
        f"- Adapter holdout: `{len(adapter_holdout_entries)}`",
        f"- All holdout: `{len(all_holdouts)}`",
        "",
        "## Decision Counts",
        "",
    ...
    print(f"Alpha360 batch promotion catalog written to {combined_path}", flush=True)
    return {"combined": combined_path, "audit": audit_path, "report": report_path}


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Promote Alpha360 candidate batch factors after V4 evaluation.")
