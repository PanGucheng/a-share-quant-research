...
def main() -> int:
    config_path = PROJECT_ROOT / "configs/full_research_trial_readiness_v1.yaml"
    ...
    assert bool(flags["full_research_trial_infrastructure_ready"])
    assert bool(flags["full_research_validation_chain_ready"])
    assert bool(flags["full_research_qlib_execution_operational"])
    assert not bool(flags["full_research_authoritative_tradability_ready"])
    assert bool(flags["full_research_trial_ready"])
    assert bool(flags["pr4_scale_up_ready"])
    assert not bool(flags["model_training_started"])
    print("All compact full-research 80-factor trial evidence validations passed.")
    return 0
