...
DEFAULT_CONFIG = Path("configs/alpha101_factor_batch_promotion_v1.yaml")
...
def update_entry(entry: dict, *, stage: str, enabled: bool, runnable: bool, suffix: str) -> dict:
    ...
    notes = notes.replace("Pending TA batch V4 evaluation.", "")
    notes = notes.replace("Pending Alpha101 batch V4 evaluation.", "").strip()
...
def run(config_path: Path) -> dict[str, Path]:
    ...
    for factor, entry in source_entries.items():
        ...
        if decision == "promoted":
            promoted_entries.append(
                update_entry(
                    entry,
                    stage=str(promotion.get("passed_stage", "alpha101_adapter_v4_batch_passed")),
                    enabled=True,
                    runnable=True,
                    suffix="Alpha101 batch V4 passed.",
                )
            )
        else:
            holdout_entries.append(
                update_entry(
                    entry,
                    stage=str(promotion.get("holdout_stage", "alpha101_adapter_v4_batch_holdout")),
                    enabled=False,
                    runnable=False,
                    suffix=f"Alpha101 batch V4 holdout: {reason}.",
                )
            )
        audit_rows.append(
            {
    ...
    common_policy = {
        ...
        "principle": [
            ...
            "Alphalens or jqfactor quantile_turnover partial failures are held out for review.",
            "Adapter zero-valid factors remain holdouts and are not sent to V4.",
        ],
    }
    write_yaml(
        {
            "version": 1,
            "updated": "2026-06-28",
            "policy": {**common_policy, "purpose": "Alpha101 batch-passed factors after candidate71 V4 evaluation."},
    ...
    write_yaml(
        {
            "version": 1,
            "updated": "2026-06-28",
            "policy": {**common_policy, "purpose": "Alpha101 V4 holdout factors after candidate71 batch evaluation."},
            "factors": holdout_entries,
        },
        holdout_path,
    )
    all_holdouts = sorted(adapter_holdout_entries + holdout_entries, key=lambda item: str(item["name"]))
    write_yaml(
        {
            "version": 1,
            "updated": "2026-06-28",
            "policy": {**common_policy, "purpose": "Combined Alpha101 holdout catalog: adapter zero-valid plus V4 holdouts."},
    ...
    write_yaml(
        {
            "version": 1,
            "updated": "2026-06-28",
            "policy": {**common_policy, "purpose": "Combined Alpha101 promoted catalog: smoke passed plus batch passed."},
            "factors": combined,
        },
        combined_path,
    )
    ...
    lines = [
        ...
        f"- V4 batch holdout: `{len(holdout_entries)}`",
    ...
    print(f"Alpha101 batch promotion catalog written to {combined_path}", flush=True)
    return {"combined": combined_path, "audit": audit_path, "report": report_path}


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Promote Alpha101 candidate batch factors after V4 evaluation.")
