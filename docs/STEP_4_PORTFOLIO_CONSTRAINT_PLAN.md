...
```text
outputs/reports/topk_scan_liquid2000_community_20260609.md
```

结果摘要：

| topk | n_drop | 成本后年化超额 | 成本后 IR | 成本后最大回撤 |
| ---: | ---: | ---: | ---: | ---: |
| 50 | 5 | `0.054915` | `0.312190` | `-0.169295` |
| 100 | 10 | `0.082686` | `0.524204` | `-0.148199` |
| 200 | 20 | `0.151621` | `1.051600` | `-0.149152` |
| 300 | 30 | `0.151472` | `1.060656` | `-0.146884` |

Interpretation:

- `topk=50` 对 `liquid2000` 仍然偏窄。
- `topk=200` 到 `topk=300` 是当前更合适的区间。
- `topk=300` 的年化超额与 `topk=200` 接近，但 IR 和回撤略优。
- 下一阶段可把 `topk=300/n_drop=30` 作为宽股票池组合基线，同时保留 `topk=200/n_drop=20` 作对照。
...
- `liquid2000` 上的 TopK 扫描表明，`topk=200` 到 `topk=300` 比 `topk=50` 更适合当前宽股票池。
