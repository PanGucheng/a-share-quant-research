...
> ARCHIVED / HISTORICAL：工具链 readiness 是已完成阶段记录，不是当前研究入口。
...
V3.37 后新增 tradability exposure attribution contract：

```text
tradability_exposure_attribution_board rows: 19
tradability_exposure_action_summary rows: 5
tradability_exposure_contract_status rows: 6
