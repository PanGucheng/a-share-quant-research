...
V3.35 后新增 Alpha360 strict OOS extension contract：

```text
alpha360_strict_oos_expression_summary rows: 3
alpha360_strict_oos_batch_manifest rows: 1
alpha360_strict_oos_metric_summary rows: 3
alpha360_strict_oos_evaluator_status rows: 9
alpha360_strict_oos_contract_status rows: 8
alpha360_strict_oos_extension: pass
overall_status: ready
```

V3.36 后新增 Alpha360 strict OOS stability contract：

```text
alpha360_strict_oos_stability_metrics rows: 54
alpha360_strict_oos_stability_summary rows: 3
alpha360_strict_oos_stability_contract_status rows: 8
alpha360_strict_oos_stability: pass
overall_status: ready
```

这表示“引入更多因子”的入口已经打开，并且已由 TA、Alpha101 与 Alpha360 三类非 seed 来源验证；第一层 probes 诊断、review、strict-OOS 复核和 main-vs-recent 稳定性也已经接入 readiness。下一阶段应推进 19 个 tradability exposure watchlist 的流动性/可交易性暴露归因，并继续推进行业/风格数据能力审计。所有扩张仍需沿用 adapter audit、V4 batch、promotion/holdout、multi-source screening、multi-source judgement、probe diagnostics、probe review 和 strict-OOS stability contract。

## 7. 下一步目标

下一步不继续围绕 Alpha158 微调，也不急着训练模型，而是进入“暴露归因 + 暴露数据能力审计”阶段：
