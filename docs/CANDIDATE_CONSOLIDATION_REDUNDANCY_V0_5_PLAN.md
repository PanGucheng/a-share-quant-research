...
用户依据独立审阅要求实施修订；2026-09-09交付[V0.5.1提案](../reports/candidate_consolidation_v0_5/v0_5_1/REPORT.md)。复用P1/P2/P3数值证据，P4仅在已确认相同机制、窗口及量纲且满足D3原有Full/四Era可比、q10和符号条件时提出近似替代。未知字符串不构成同一语义层，运行说明不作为公式复杂度；低覆盖单例显式披露未知pair。五组精确样本等值单独登记，不改变库存身份或Primary方向。

本轮包括TA实际参数与canonical KAMA覆盖核验、PIT/TTM口径分离、ATR/ADX的合成和少量开发期真实缺失诊断。价格provider尺度未核定的提案继续暂缓。原版本和receipt保留，新版本独立发布；不重跑全量日相关，不按目标数量/收益调整cut，不进入Core、2024+诊断、模型或Strategy V2。
