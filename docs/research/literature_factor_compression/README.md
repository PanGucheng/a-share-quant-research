...
2026-09-10 的计划 V0.2 只修订设计：六项正式 contrasts、n_min/50% 的结构诊断要求及 D1 验收边界。
本目录 CSV/JSON 仍是 V0.1 快照，未因附件提出 D1 实施要求而被改成已审核角色。
采纳与修正记录见主计划第 21 节。随后用户已授权先提交计划再实施 D1；
本目录原快照仍保持不变，D1 结果另行发布。
